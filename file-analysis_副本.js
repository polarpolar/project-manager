...
const QUOTE_ANALYZE_PROMPT = `你是报价分析助手。请阅读这份方案报价，提取总报价金额（单位：万元）和交付内容，按以下格式返回JSON，不加任何说明文字：
...
示例：{"quote":"128.5","brief":"无线振动传感器×20套，数据采集网关×2套","wireless_hardware":true,"wired_hardware":false,"software":true,"other":""}`;
...
async function loadModalFilePanel(projectId) {
  ...
  // 恢复之前的识别结果
  if (p._aiAnalysisResult) {
    const { type, result, analysisDivId } = p._aiAnalysisResult;
...
function renderQuoteAnalysis(data, analysisDiv, projectId) {
  try {
    ...
    // 构建交付内容标签
    const tags = [];
    if (result.brief) tags.push(result.brief);
    if (result.wireless_hardware) tags.push('无线硬件');
    if (result.wired_hardware) tags.push('有线硬件');
    if (result.software) tags.push('软件');
    if (result.other) tags.push(result.other);
    const deliveryTags = {
      brief: result.brief || '',
      wireless_hardware: result.wireless_hardware || false,
      wired_hardware: result.wired_hardware || false,
      software: result.software || false,
      other: result.other || ''
    };
